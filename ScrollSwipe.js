...
(function($){ //Dependencies: jQuery (Built on 1.8.2)
	$.fn.ScrollSwipe = function(options){
		var _options = $.extend({}, $.fn.ScrollSwipe.defaults, options);
		...
		var _methods = {
			initialize:function(){
				if (!_options.scrollEasing){
					_options.scrollEasing = ScrollSwipe.expoEaseOut; //Default
				}
				...
				_dragNormalTimer = ScrollSwipe.NormalTimer();
				_renderNormalTimer = ScrollSwipe.NormalTimer();
			...
			scrollToChild:function(selectChild, tween){ //Selects child
				...
				if (typeof(selectChild.data("ScrollSwipe.index")) === typeof(undefined)){
					_methods.updateChildren();
				}
				...
				var newIndex = selectChild.data("ScrollSwipe.index");
			...
			updateChildren:function(){
				...
				for (var i = 0; i < childrenLen; i++){
					$(_children[i]).data("ScrollSwipe.index", i);
				}
				if (_options.scrollbars){
			...
			buildScrollbars:function(){
				_scrollbarHMask = $("<div class='ScrollSwipe-scrollbar-mask ScrollSwipe-scrollbar-h-mask'style='display:none;overflow:hidden;position:absolute;bottom:1px;left:1px;width:100%;height:4px;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;'></div>");
				_scrollbarH = $("<div class='ScrollSwipe-scrollbar ScrollSwipe-scrollbar-h'style='position:absolute;left:0;bottom:0;background-color:rgba(0,0,0,0.6);min-width:4px;height:100%;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;'></div>");
				_scrollbarHMask.append(_scrollbarH);
				_parent.append(_scrollbarHMask);
				_scrollbarVMask = $("<div class='ScrollSwipe-scrollbar-mask ScrollSwipe-scrollbar-v-mask'style='display:none;overflow:hidden;position:absolute;right:1px;top:1px;width:4px;height:100%;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;'></div>");
				_scrollbarV = $("<div class='ScrollSwipe-scrollbar ScrollSwipe-scrollbar-v'style='position:absolute;right:0;top:0;background-color:rgba(0,0,0,0.6);width:100%;min-height:4px;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;'></div>");
			...
			handler_moveDrag:function(evt){
				...
				if (!_isRendering){ //Threshold not met
					if (Math.sqrt(dragDiffX * dragDiffX + dragDiffY * dragDiffY) >= _options.dragThreshold){
						if (Math.abs(dragDiffX) >= Math.abs(dragDiffY)){ //Dragged horizontal
							_dragDirection = ScrollSwipe.DIRECTION_HORIZONTAL;
							if (elementWidth <= parentWidth){ //Can't scroll horizontally
								_isPageScrolling = true;
							}
						} else { //Dragged vertical
							_dragDirection = ScrollSwipe.DIRECTION_VERTICAL;
							if (elementHeight <= parentHeight){ //Can't scroll vertically
								_isPageScrolling = true;
							}
						}
				...
				if (_dragDirection == ScrollSwipe.DIRECTION_HORIZONTAL || _options.bidirectional){ //Dragged horizontal
					if (!_isPageScrolling){
				...
				if (_dragDirection == ScrollSwipe.DIRECTION_VERTICAL || _options.bidirectional){ //Dragged vertical
					if (!_isPageScrolling){
			...
			beginTween:function(fromPoint){
				...
				if (!_isPageScrolling){
					$(_element).trigger(ScrollSwipe.EVENT_CHANGING, {
			...
			fireEventChanged:function(){
				...
				$(_element).trigger(ScrollSwipe.EVENT_CHANGED, {
					selectedChild:_selectedChild,
					selectedIndex:_selectedIndex
				});
				_indexChanged = false;
		...
		this.ScrollSwipe = { //Expose public methods
	...
	$.fn.ScrollSwipe.defaults = ScrollSwipe.defaults;
}(jQuery));