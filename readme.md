...
*The following is an example for Swipeable marquees. 
...
```
`scroller` is what gets targeted using jQuery to apply ScrollSwipe.
`scrollerWrapper` acts as a mask.
When snapping is enabled for the ScrollSwipe instance the top-left of `scroller` children will snap with the top-left of `scrollerWrapper`.
...
```JavaScript
	$(document).on("ready", function(){
		...
		var scroller = $("#scroller").ScrollSwipe({ 	//Initialize ScrollSwipe returns jQuery instance
